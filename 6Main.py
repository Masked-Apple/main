...
cur = conn.cursor()
con= sqlite3.connect('snipedata.sqlite')
curr = con.cursor()
...
async def on_ready():
    print("✔ Connected!\n")
    client.load_extension("libneko.extras.superuser")
    print("Loaded cog")


@client.event
async def on_message_delete(message):
    curr.execute("SELECT channel FROM snipe WHERE channel = ?", (message.channel.id,))
    row = curr.fetchone()
    strow = str(row)
    if strow is None:
        return print("yikes")
    else:
        curr.execute('DELETE FROM "snipe" WHERE channel=?', (str(message.channel.id),))
        con.commit()
        pfp = message.guild.get_member(message.author.id).avatar_url
        curr.execute('INSERT INTO "snipe" (channel, name, pfp, message) values (?, ?, ?, ?)', (str(message.channel.id), str(message.author.name), str(pfp) , str(message.content)))
        con.commit()
...
async def say(ctx):
    if ctx.author.id == owner:
        ...
        ez = await guild.get_channel(ctx.message.content.split()[1])
        await ez.send("%s" % (" ".join(ctx.message.content.split(" ")[2:])))
...
@client.command()
async def drop(ctx):
    example = discord.utils.get(ctx.guild.roles, name='Giveaways')
    if example in [role for role in ctx.author.roles]:
...
@client.command()
async def space(ctx, channel: discord.TextChannel=None):
    if ctx.author.guild_permissions.manage_channels or ctx.author.id == 575141146037321729:
        if channel is None:
            chan = ctx.channel
        else:
            chan = channel
        if "-" in chan.name:
            try:
                new = chan.name.replace("-", "  ")
                await chan.edit(name=new)
                await ctx.channel.send(f"**Done!** <#{chan.id}> now has spaces.")
            except discord.Forbidden:
                await ctx.channel.send("**Oh my god.. you really are a tard** How do you expect me to edit channel names if I don't have the correct permissions??\nYou should find a role called \"Apple Bot\"\nhttps://gyazo.com/0ba731a73903c3e4382d0bc1d6e467ca")
        else:
            await ctx.channel.send(f"**No.. idiot** <#{chan.id}> needs to have \"**-**\" in it in order for me to replace it with a space. Put a **-** in it then do  the command..\nE.g: https://bit.ly/2WLKs9b")
    else:
        await ctx.channel.send("You don't have the correct permissions to be using this command")


@client.command()
async def snipe(ctx):
        curr.execute("SELECT channel FROM snipe WHERE channel = ?", (ctx.channel.id,))
        row = curr.fetchone()
        strow = str(row)
        if strow is None:
            return await ctx.channel.send("Theres nothing to snipe!")
        curr.execute("SELECT message FROM snipe WHERE channel = ?", (ctx.channel.id,))
        conn = str(curr.fetchone()[0])
        embed= discord.Embed(colour=discord.Colour.green(), description=conn)
        curr.execute("SELECT name FROM snipe WHERE channel = ?", (ctx.channel.id,))
        name = str(curr.fetchone()[0])
        curr.execute("SELECT pfp FROM snipe WHERE channel = ?", (ctx.channel.id,))
        pfp = str(curr.fetchone()[0])
        embed.set_author(name=name, icon_url=pfp)
        embed.set_footer(text="Snaking since 2018")
        await ctx.channel.send(embed=embed)
